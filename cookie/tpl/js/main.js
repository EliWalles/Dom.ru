...
Game.Launch=function()
{
	...
	{
		Game.ready=1;
		l('javascriptError').innerHTML='<div style="padding:64px 128px;"><div class="title">Загрузка...</div></div>';
		...
		Game.WriteSave=function(exporting)//guess what we'e using to save the game?
		{
			...
			else
			{
				...
				if (document.cookie.indexOf('CookieClickerGame')<0) Game.Popup('Ошибка сохранения!');
				else Game.Popup('Игра сохранена');
			}
		}
		...
		Game.Object=function(name,commonName,desc,pic,icon,background,price,cps,drawFunction,buyFunction)
		{
			...
			{
				var str='<div class="row" id="row'+this.id+'"></div>';
		...
		Game.RebuildStore=function()//redraw the store from scratch
		{
			...
			for (var i in Game.Objects)
			{
				var me=Game.Objects[i];
				if (me.name=='Cursor') {
					me.displayName='Секретарша';
				}
				if (me.name=='Grandma') {
					me.displayName='Менеджер';
					me.icon='man';
				}
				if (me.name=='Farm') {
					me.displayName='Телефон';
					me.icon='telmap';
				}
				if (me.name=='Factory') {
					me.displayName='Интернет';
					me.icon='int';
				}
				if (me.name=='Mine') {
					me.displayName='Выделенная линия';
					me.icon='phonewall';
				}
				if (me.name=='Shipment') {
					me.displayName='Видеонаблюдение';
					me.icon='vide1';
				}
				if (me.name=='Alchemy lab') {
		...
		//define objects
		new Game.Object('Cursor','cursor|cursors|clicked','Отвечает на звонки каждые 10 секунд','cursor','woman','',15,function(){
			var add=0;
			if (Game.Has('Thousand fingers')) add+=0.1;
			if (Game.Has('Million fingers')) add+=0.5;
			if (Game.Has('Billion fingers')) add+=2;
	...
	setInterval(function fraza() {
		...
		$('#talk').empty();
		$('#talk').fadeIn('slow', function() {
			var trand = getRandomArbitary(0, ml);
    		$('#talk').html(fraz[trand]);
    		rand = getRandomArbitary(10000, 30000);
    		setTimeout(function() {
    			$('#talk').fadeOut('slow');
    		}, 5000);
  		});
		...
		new Game.Object('Farm','farm|farms|harvested','Grows cookie plants from cookie seeds.','farm','farmIcon','farmBackground',500,function(){
			...
			if (this.amount>=1) {
			 $('#tree').fadeIn('slow');$('#garbage').fadeIn('slow'); Game.Win('My first farm');if (this.amount>=50) Game.Win('Reap what you sow');if (this.amount>=100) Game.Win('Farm ill');
			 }
		});
		...
		new Game.Object('Factory','factory|factories|mass-produced','Produces large quantities of cookies.','factory','factoryIcon','factoryBackground',3000,function(){
			...
			if (this.amount>=1) {
			$('#poster').fadeIn('slow');$('#r2d2').fadeIn('slow');$('#flower').fadeIn('slow'); $('#stel2').fadeIn('slow'); Game.Win('Production chain');if (this.amount>=50) Game.Win('Industrial revolution');if (this.amount>=100) Game.Win('Global warming');
			}
		});
		...
		new Game.Object('Mine','mine|mines|mined','Mines out cookie dough and chocolate chips.','mine','mineIcon','mineBackground',10000,function(){
			...
			if (this.amount>=1) {
			 $('#folder').fadeIn('slow');Game.Win('You know the drill');if (this.amount>=50) Game.Win('Excavation site');if (this.amount>=100) Game.Win('Hollow the planet');
			 }
		});
		...
		new Game.Object('Shipment','shipment|shipments|shipped','Brings in fresh cookies from the cookie planet.','shipment','shipmentIcon','shipmentBackground',40000,function(){
			...
			if (this.amount>=1) {
			 $('#video').fadeIn('slow');$('#cat').fadeIn('slow'); $('#ball1').fadeIn('slow'); $('#ball2').fadeIn('slow');Game.Win('Expedition');if (this.amount>=50) Game.Win('Galactic highway');if (this.amount>=100) Game.Win('Far far away');
			 }
		});
		...
		new Game.Object('Alchemy lab','alchemy lab|alchemy labs|transmuted','Turns gold into cookies!','alchemylab','alchemylabIcon','alchemylabBackground',200000,function(){
			...
			if (this.amount>=1){
			 $('#phoneTable').fadeIn('slow');$('#ball3').fadeIn('slow'); Game.Win('Transmutation');if (this.amount>=50) Game.Win('Transmogrification');if (this.amount>=100) Game.Win('Gold member');
			 }
		});
	...
	Game.Logic=function()
	{
		...
		if (Game.T%(Game.fps)==0) {
		 ...
		 document.title=Beautify(Game.cookies)+' '+(Game.cookies==1?'рубль':'рублей')+' - Дом.ру кликер';
		}
